...
class ItemRepositoryTest < Minitest::Test
  ...
  def test_read_data_returns_item_instances
    ...
    assert_equal "Parker", seventh_item.first_name
    assert_equal "Daugherty", seventh_item.last_name
    assert_equal "2012-03-27 14:54:10 UTC", seventh_item.created_at
    assert_equal "2012-03-27 14:54:10 UTC", seventh_item.updated_at
  end

  def test_read_data_returns_all_instances
    assert_equal 11, item_repo.all.length
  end
  ...
  def test_can_find_item_by_first_name
    expected = "Dejon"
    assert_equal expected, item_repo.find_by_first_name("Dejon").first_name
  end

  def test_can_find_item_by_first_name_case_insensitive
    expected = "Dejon"
    assert_equal expected, item_repo.find_by_first_name("dejon").first_name
  end

  def test_can_find_item_by_last_name
    expected = "Toy"
    assert_equal expected, item_repo.find_by_last_name("Toy").last_name
  end

  def test_can_find_item_by_las_name_case_insensitive
    expected = "Nader"
    assert_equal expected, item_repo.find_by_last_name("nader").last_name
  end

  def test_does_not_return_name_when_only_part_matches
    expected = nil
    assert_equal expected, item_repo.find_by_first_name("Ramon")
  end

  def test_no_first_name
    assert_equal nil, item_repo.find_by_first_name(nil)
  end

  def test_no_last_name
    assert_equal nil, item_repo.find_by_last_name(nil)
  end

  def test_can_find_item_by_time_created
    expected = "2012-03-27 14:54:09 UTC"
    assert_equal expected, item_repo.find_by_created_at("2012-03-27 14:54:09 UTC").created_at
  end

  def test_can_find_item_by_time_updated
    expected = "2012-03-27 14:54:11 UTC"
    assert_equal expected, item_repo.find_by_updated_at("2012-03-27 14:54:11 UTC").updated_at
  end

  def test_can_find_all_items_by_id
    assert_equal 1, item_repo.find_all_by_id(2).length
    assert_equal 0, item_repo.find_all_by_id(12).length
  end

  def test_can_find_all_items_by_first_name
    assert_equal 2, item_repo.find_all_by_first_name("Mariah").length
    assert_equal 0, item_repo.find_all_by_first_name("David and Rose").length
  end
